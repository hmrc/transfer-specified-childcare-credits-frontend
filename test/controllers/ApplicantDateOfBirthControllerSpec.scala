...
import pages.{ApplicantDateOfBirthPage, ApplicantNamePage}
...
class ApplicantDateOfBirthControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider = new ApplicantDateOfBirthFormProvider(clock)

  def onwardRoute = Call("GET", "/foo")

  val validAnswer = LocalDate.now(ZoneOffset.UTC).minusYears(16)
  ...
  private def form = formProvider(applicantName)
  ...
  "ApplicantDateOfBirth Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(form, applicantName, NormalMode)(getRequest, messages(application)).toString
      }
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers = minimalUserAnswers.set(ApplicantDateOfBirthPage, validAnswer).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(validAnswer), applicantName, NormalMode)(getRequest, messages(application)).toString
      }
    }
