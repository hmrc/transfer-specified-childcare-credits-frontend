...
import forms.ApplicantChildcareAfterCutoffFormProvider
import models.{ChildName, NormalMode, UserAnswers}
import navigation.{FakeNavigator, Navigator}
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.{ApplicantChildcareAfterCutoffPage, ChildNamePage}
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.ApplicantChildcareAfterCutoffView

import scala.concurrent.Future

class ApplicantChildcareAfterCutoffControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new ApplicantChildcareAfterCutoffFormProvider()
  val form = formProvider()

  lazy val applicantChildcareAfterCutoffRoute = routes.ApplicantChildcareAfterCutoffController.onPageLoad(NormalMode).url

  val childName = ChildName("Foo", "Bar")
  val minimalUserAnswers = emptyUserAnswers.set(ChildNamePage, childName).success.value

  "ApplicantChildcareAfterCutoff Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, applicantChildcareAfterCutoffRoute)
        val result = route(application, request).value
        val view = application.injector.instanceOf[ApplicantChildcareAfterCutoffView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = minimalUserAnswers.set(ApplicantChildcareAfterCutoffPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, applicantChildcareAfterCutoffRoute)
