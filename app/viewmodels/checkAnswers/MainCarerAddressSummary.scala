...
object MainCarerAddressSummary  {
    ...
    answers.get(MainCarerAddressPage).map {
      ...
      val value = HtmlFormat.escape(answer.line1).toString + "<br/>" + HtmlFormat.escape(answer.line2).toString
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.MainCarerAddressController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("mainCarerAddress.change.hidden"))
          )
        )
    }
}
