...
object ApplicantAddressSummary  {
    ...
    answers.get(ApplicantAddressPage).map {
      ...
      val value = HtmlFormat.escape(answer.line1).toString + "<br/>" + HtmlFormat.escape(answer.line2).toString
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.ApplicantAddressController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("applicantAddress.change.hidden"))
          )
        )
    }
}
